...
public class Graph {
    ...
    public void addEdge(String origen, String destino, double peso) throws Exception{
        ...
        Edge arista =new Edge((Vertex)vertices.get(indice1),(Vertex)vertices.get(indice2),peso);
        pesos.add(arista);
	 ...
	 public double getPeso(String origen, String destino){
        ...
        for (int x=0;x<pesos.size();x++){
            if(aux.equals((Edge)pesos.get(x))){
                aux=(Edge)pesos.get(x);
                return aux.getPeso();
            }
        }
    ...
    public void crearMatriz(){
        ...
        for (int x=0;x<vertices.size();x++){
            for(int j=0;j<vertices.size();j++){
                ...
                adyacencia[x][j]=getPeso(((Vertex)vertices.get(x)).getNombre(),((Vertex)vertices.get(j)).getNombre());
            }
        }
    }   
